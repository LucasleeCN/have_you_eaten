...
import com.example.have_you_eaten.lsy_activity.BuyerInShopActivity;
...
public class MenuActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void getShop() {
        ...
        Request request = new Request.Builder().addHeader("Authorization", AppStatus.token)
                ...
                .url(ApiUrl.AUTHER_GET_SHOPINFO)//更改哦
                .get()
                .build();
            ...
            switch (msg.what) {
                case GETMENU_SUCCESS:
                    ...
                    for(int i = 0;i<jsonArray.size();i++){
                        Shop shops = new Shop();
                        shops.setName(jsonArray.getJSONObject(i).getString("name"));
                        shops.setAddress(jsonArray.getJSONObject(i).getString("address"));
                        shops.setId(jsonArray.getJSONObject(i).getLong("id"));
                    ...
                    //TODO 跳转至FoodMenuActivity
                    listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                        @Override
                        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                            Shop shop=menuList_item.get(position);
                            Intent intent=new Intent(MenuActivity.this, BuyerInShopActivity.class);
